...
# this is updated for a "fix GAMS" effort
...
gam_function <- function(x){
  ...
  if(length(unique(filter(bd,count>0)$doy)) >= 3){
    ...
    baca_gam <- gam(count ~ s(doy, k = 30, bs = "cr") + s(lunar.phase, k = 3, bs = "cr"), 
                    data = bd)
    ...
    write.csv(x = baca_df, file = paste0("gamOutputsCSV_Fixed/", "GAM_Baca_", bw, ".csv"), row.names = F)
  ...
  if(length(unique(filter(jd,count>0)$doy)) >= 3){
    ...
    joma_gam <- gam(count ~ s(doy, k = 30, bs = "cr") + s(lunar.phase, k = 3, bs = "cr"), 
                    data = jd)
    ...
    write.csv(x = joma_df, file = paste0("gamOutputsCSV_Fixed/", "GAM_Joma_", bw, ".csv"), row.names = F)
  ...
  if(length(unique(filter(cd,count>0)$doy))){
    ...
    cofr_gam <- gam(count ~ s(doy, k = 30, bs = "cr") + s(lunar.phase, k = 3, bs = "cr"), 
    ...
    write.csv(x = cofr_df, file = paste0("gamOutputsCSV_Fixed/", "GAM_Cofr_", bw, ".csv"), row.names = F)
  ...
  if(length(unique(filter(bid,count>0)$doy))){
    ...
    biva_gam <- gam(count ~ s(doy, k = 30, bs = "cr") + s(lunar.phase, k = 3, bs = "cr"), 
    ...
    write.csv(x = biva_df, file = paste0("gamOutputsCSV_Fixed/", "GAM_Biva_", bw, ".csv"), row.names = F)
  ...
  if(length(unique(filter(bod,count>0)$doy))){
    ...
    bowa_gam <- gam(count ~ s(doy, k = 30, bs = "cr") + s(lunar.phase, k = 3, bs = "cr"), 
    ...
    write.csv(x = bowa_df, file = paste0("gamOutputsCSV_Fixed/", "GAM_Bowa_", bw, ".csv"), row.names = F)
  ...
  if(length(unique(filter(dod,count>0)$doy))){
    demi_gam <- gam(count ~ s(doy, k = 30, bs = "cr") + s(lunar.phase, k = 3, bs = "cr"), 
    ...
    write.csv(x = demi_df, file = paste0("gamOutputsCSV_Fixed/", "GAM_Demi_", bw, ".csv"), row.names = F)
  ...
  if(length(unique(filter(rid,count>0)$doy))){
    rist_gam <- gam(count ~ s(doy, k = 30, bs = "cr") + s(lunar.phase, k = 3, bs = "cr"), 
    ...
    write.csv(x = rist_df, file = paste0("gamOutputsCSV_Fixed/", "GAM_Rist_", bw, ".csv"), row.names = F)
  ...
  if(length(unique(filter(prd,count>0)$doy))){
    prcr_gam <- gam(count ~ s(doy, k = 30, bs = "cr") + s(lunar.phase, k = 3, bs = "cr"), 
    ...
    write.csv(x = prcr_df, file = paste0("gamOutputsCSV_Fixed/", "GAM_Prcr_", bw, ".csv"), row.names = F)
  ...
  if(length(unique(filter(aud,count>0)$doy))){
    auca_gam <- gam(count ~ s(doy, k = 30, bs = "cr") + s(lunar.phase, k = 3, bs = "cr"), 
    ...
    write.csv(x = auca_df, file = paste0("gamOutputsCSV_Fixed/", "GAM_Auca_", bw, ".csv"), row.names = F)
  ...
  ggsave(filename = paste0("gamOutputs_lunar_FixedAttempts/", bw, ".png"), plot = cp, 
         width = 7, height = 7)
  
  
}

lapply(X = "Symmerista albifrons", FUN = gam_function)
